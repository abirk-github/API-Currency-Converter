...
# For API request code verification purposes
@app.route('/currency_converter/<amount>/<input_currency>/<output_currency>', methods= ["GET", "POST", "PUT"])
...
# For header validation test purposes
@app.route('/currency_converter/<amount>/<input_currency>/<output_currency>', methods= ["GET", "POST", "PUT"])
def get_header():
    with app.test_request_context():
        if request.method == 'GET':
            print('Get headers:' ,request.headers.get('Content-Length'))
            return request.headers.get('Content-Length')
        else:
            return 'Test not initiated.'
...
class CurrencyConverter(Resource):
    def get(self):
        ...
        return test_currency_converter_api(amount, input_currency, output_currency)
